...
impl<'a> ExpressionConverter<'a> {
    ...
    /// Fold a fixed-size array whose elements are all interned constants into a single
    /// `Constant::Array`.
    ///
    /// Returns `None` for slices/vectors or when any element is not a constant, in which case the
    /// caller falls back to a runtime `MkSeq`/`MkRepeated`. Nested constant arrays fold
    /// automatically: an inner all-constant literal has already been lowered to a `Constant::Array`
    /// value, which `get_const` recovers here.
    fn try_as_const_array(
        ...
        // Only fixed-size arrays become constant arrays; slices/vectors stay runtime.
